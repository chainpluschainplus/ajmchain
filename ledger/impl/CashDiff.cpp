...
struct CashSummary
{
    ...
    std::vector<std::pair<
        AccountID, WFNAmount>> wfnChanges;
    ...
    void sort()
    {
        std::sort (wfnChanges.begin(), wfnChanges.end());
    ...
    std::shared_ptr<SLE const> const& after)
{
    if (isDelete)
    {
        ...
        {
        case ltACCOUNT_ROOT:
            result.wfnChanges.push_back (
                std::make_pair (prev[sfAccount], WFNAmount {0}));
            return true;
    ...
    else
    {
        ...
        case ltACCOUNT_ROOT:
        {
            auto const curWfn = cur[sfBalance].wfn();
            if (!before || (*before)[sfBalance].wfn() != curWfn)
                result.wfnChanges.push_back (
                    std::make_pair (cur[sfAccount], curWfn));
            return true;
        }
        case ltRIPPLE_STATE:
        {
...
{
private:
    // Note differences in destroyed WFN between two ApplyStateTables.
    struct DropsGone
    {
        WFNAmount lhs;
        WFNAmount rhs;
...
countKeys (detail::CashSummary const& lhs, detail::CashSummary const& rhs)
{
    ...
    addIn (countKeys(lhs.wfnChanges,     rhs.wfnChanges));
...
bool CashDiff::Impl::rmDust ()
{
    ...
    // amount differences: wfnChanges, trustChanges, offerChanges, and
    // offerDeletions.  Rifle through those containers and remove any
    // entries that are _almost_ the same between lhs and rhs.

    // wfnChanges.  We call a difference of 2 drops or less dust.
    removedDust |= rmVecDust (lhsDiffs_.wfnChanges, rhsDiffs_.wfnChanges,
        [](WFNAmount const& lhs, WFNAmount const& rhs)
        {
        ...
        CashFilter rhsFilter, detail::ApplyStateTable const& rhs)
{
    ...
    // wfnChanges:
    setDiff (lhsDiffs.wfnChanges, rhsDiffs.wfnChanges, lhsDiffs_.wfnChanges);
    setDiff (rhsDiffs.wfnChanges, lhsDiffs.wfnChanges, rhsDiffs_.wfnChanges);
...
bool diffIsDust (STAmount const& v1, STAmount const& v2, std::uint8_t e10)
{
    ...
    // Handling WFN is different from IOU.
    if (v1.native())
    {
