              ...
              beast::Journal j)
{
    ...
    if (isWFN(currency))
    {
        return {wfnLiquid (view, account, 0, j)};
    }
...
WFNAmount
wfnLiquid (ReadView const& view, AccountID const& id,
    std::int32_t ownerCountAdj, beast::Journal j)
{
    ...
    if (fix1141 (view.info ().parentCloseTime))
    {
        ...
        auto const balance = view.balanceHook(id, wfnAccount(), fullBalance);
    ...
    {
        // pre-switchover
        // WFN: return balance minus reserve
        ...
        return view.balanceHook(id, wfnAccount(), amount).wfn();
    }
}
    ...
    beast::Journal j)
{
    ...
    assert (!isWFN (uSenderID) && uSenderID != noAccount());
    assert (!isWFN (uReceiverID) && uReceiverID != noAccount());
    ...
    STAmount const& saAmount, STAmount& saActual, beast::Journal j)
{
    ...
    assert (!isWFN (uSenderID) && !isWFN (uReceiverID));
    assert (uSenderID != uReceiverID);
    ...
    STAmount const& saAmount, beast::Journal j)
{
    ...
    /* WFN send which does not check reserve and can do pure adjustment.
    ...
    if (sender)
    {
        ...
        else
        {
            ...
            if (fv2Switch)
                view.creditHook (uSenderID, wfnAccount (), saAmount, sndBal);

            // Decrement WFN balance.
    ...
    {
        // Increment WFN balance.
        auto const rcvBal = receiver->getFieldAmount (sfBalance);
        receiver->setFieldAmount (sfBalance, rcvBal + saAmount);

        if (fv2Switch)
            view.creditHook (wfnAccount (), uReceiverID, saAmount, -rcvBal);
...
{
    assert (!isWFN (account) && !isWFN (issue.account));
