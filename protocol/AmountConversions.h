...
inline
STAmount
toSTAmount (WFNAmount const& wfn)
{
    bool const isNeg = wfn.signum() < 0;
    std::uint64_t const umant = isNeg ? - wfn.drops () : wfn.drops ();
    return STAmount (umant, isNeg);
}

inline
STAmount
toSTAmount (WFNAmount const& wfn, Issue const& iss)
{
    assert (isWFN(iss.account) && isWFN(iss.currency));
    return toSTAmount (wfn);
}
...
inline
WFNAmount
toAmount<WFNAmount> (STAmount const& amt)
{
    assert (amt.mantissa () < std::numeric_limits<std::int64_t>::max ());
    bool const isNeg = amt.negative ();
    std::int64_t const sMant =
