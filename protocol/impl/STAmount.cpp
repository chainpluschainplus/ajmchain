...
STAmount::STAmount (WFNAmount const& amount)
    : mOffset (0)
    , mIsNative (true)
    , mIsNegative (amount < zero)
{
...
//
//------------------------------------------------------------------------------
WFNAmount STAmount::wfn () const
{
    if (!mIsNative)
        Throw<std::logic_error> ("Cannot return non-native STAmount as WFNAmount");
...
amountFromString (Issue const& issue, std::string const& amount)
{
    ...
    // Can't specify WFN using fractional representation
    if (isWFN(issue) && match[3].matched)
        Throw<std::runtime_error> ("WFN must be specified in integral drops.");
...
amountFromJson (SField const& name, Json::Value const& v)
{
    ...
    {
        if (v.isObject ())
            Throw<std::runtime_error> ("WFN may not be specified as an object");
        issue = wfnIssue ();
    }
    else
    {
...
multiply (STAmount const& v1, STAmount const& v2, Issue const& issue)
{
    ...
    if (v1.native() && v2.native() && isWFN (issue))
    {
    ...
    bool roundUp)
{
    ...
    bool const wfn = isWFN (issue);

    if (v1.native() && v2.native() && wfn)
    {
    ...
    if (resultNegative != roundUp)
        canonicalizeRound (wfn, amount, offset);
    ...
    {
        if (wfn && *stAmountCalcSwitchover2)
        {
    ...
    Issue const& issue, bool roundUp)
{
    ...
    if (resultNegative != roundUp)
        canonicalizeRound (isWFN (issue), amount, offset);

    STAmount result (issue, amount, offset, resultNegative);
    // Control when bugfixes that require switchover dates are enabled
    if (roundUp && !resultNegative && !result && *stAmountCalcSwitchover)
    {
        if (isWFN(issue) && *stAmountCalcSwitchover2)
        {
            // return the smallest value above zero
            amount = 1;
            offset = 0;
