    ...
    , journal_ (journal)
{
    ...
    // flavor we expect:
    assert (cross_type != CrossType::WfnToIou ||
        (isWFN (issue_in ()) && !isWFN (issue_out ())));

    assert (cross_type != CrossType::IouToWfn ||
        (!isWFN (issue_in ()) && isWFN (issue_out ())));

    // And make sure we're not crossing WFN for WFN
    assert (!isWFN (issue_in ()) || !isWFN (issue_out ()));
    ...
    Rate const& rate_out)
{
    ...
    // of an autobridge.
    if (cross_type_ == CrossType::WfnToIou && (remaining_.in < f.order.in))
    {
    ...
    Rate const& rate_in)
{
    ...
    if (!sell_ && cross_type_ == CrossType::IouToWfn)
    {
...
BasicTaker::do_cross (Amounts offer, Quality quality, AccountID const& owner)
{
    ...
    if (cross_type_ == CrossType::WfnToIou)
    {
        result = flow_wfn_to_iou (offer, quality, owner_funds, taker_funds,
            out_rate (owner, account ()));
    }
    else if (cross_type_ == CrossType::IouToWfn)
    {
        result = flow_iou_to_wfn (offer, quality, owner_funds, taker_funds,
    ...
    Amounts offer2, Quality quality2, AccountID const& owner2)
{
    ...
    // The amount available to flow via WFN is the amount that the owner of the
    ...
    // of WFN that can flow between the two legs is, essentially, infinite
    // since all the owner is doing is taking out WFN of his left pocket
    // and putting it in his right pocket. In that case, we set the available
    // WFN to the largest of the two offers.
    auto wfn_funds = get_funds (owner1, offer1.out);

    if (owner1 == owner2)
    {
        JLOG(journal_.trace()) <<
            "The bridge endpoints are owned by the same account.";
        wfn_funds = std::max (offer1.out, offer2.in);
    }
    ...
    if (auto stream = journal_.debug())
    {
        ...
        stream << "      wfn: " << format_amount (wfn_funds);
    }
    ...
    auto flow1 = flow_iou_to_wfn (offer1, quality1, wfn_funds, leg1_in_funds, leg1_rate);

    if (!flow1.sanity_check ())
        Throw<std::logic_error> ("Computed flow1 fails sanity check.");

    auto flow2 = flow_wfn_to_iou (offer2, quality2, leg2_out_funds, wfn_funds, leg2_rate);
    ...
    // equalize them, so that the amount of WFN that flows out of the first leg
    // is the same as the amount of WFN that flows into the second leg. We take
...
TER Taker::transferWFN (
    AccountID const& from,
    AccountID const& to,
    STAmount const& amount)
{
    if (!isWFN (amount))
        Throw<std::logic_error> ("Using transferWFN with IOU");
    ...
    return ripple::transferWFN (view_, from, to, amount, journal_);
}
...
{
    if (isWFN (amount))
        Throw<std::logic_error> ("Using redeemIOU with WFN");
...
{
    if (isWFN (amount))
        Throw<std::logic_error> ("Using issueIOU with WFN");
...
Taker::fill (BasicTaker::Flow const& flow, Offer& offer)
{
    ...
    if (cross_type () != CrossType::WfnToIou)
    {
        assert (!isWFN (flow.order.in));
    ...
    else
    {
        ...
        if (result == tesSUCCESS)
            result = transferWFN (account (), offer.owner (), flow.order.in);
    }

    // Now send funds from the account whose offer we're taking
    if (cross_type () != CrossType::IouToWfn)
    {
        assert (!isWFN (flow.order.out));
    ...
    else
    {
        ...
        if (result == tesSUCCESS)
            result = transferWFN (offer.owner (), account (), flow.order.out);
    }
    ...
    BasicTaker::Flow const& flow2, Offer& leg2)
{
    ...
    // leg1 to leg2: bridging over WFN
    if (result == tesSUCCESS)
        result = transferWFN (leg1.owner (), leg2.owner (), flow1.order.out);
...
{
    // In direct crossings, at least one leg must not be WFN.
    if (isWFN (offer.amount ().in) && isWFN (offer.amount ().out))
        return tefINTERNAL;
...
{
    // In bridged crossings, WFN must can't be the input to the first leg
    // or the output of the second leg.
    if (isWFN (leg1.amount ().in) || isWFN (leg2.amount ().out))
        return tefINTERNAL;
...
Taker::calculateRate (
    ApplyView const& view,
        AccountID const& issuer,
            AccountID const& account)
{
    return isWFN (issuer) || (account == issuer)
        ? parityRate
