...
CreateOffer::preflight (PreflightContext const& ctx)
{
    ...
    {
        JLOG(j.debug()) <<
            "Malformed offer: redundant (WFN for WFN)";
        return temBAD_OFFER;
    }
    ...
    // We don't allow a non-native currency to use the currency code WFN.
    ...
    NetClock::time_point const when)
{
    ...
    assert (!isWFN (takerAmount.in) && !isWFN (takerAmount.out));

    if (isWFN (takerAmount.in) || isWFN (takerAmount.out))
        Throw<std::logic_error> ("Bridging with WFN and an endpoint.");
    ...
    OfferStream offers_leg1 (view, view_cancel,
        Book (taker.issue_in (), wfnIssue ()),
        when, stepCounter_, j_);

    OfferStream offers_leg2 (view, view_cancel,
        Book (wfnIssue (), taker.issue_out ()),
        when, stepCounter_, j_);
    ...
    Amounts const& takerAmount)
{
    ...
    //
    // We check this in preclaim, but when selling WFN
    ...
    Amounts const& takerAmount)
{
    try
    {
        ...
        //
        // We check this in preclaim, but when selling WFN charged fees can
        ...
        // of the takerAmount currencies are WFN then we cross through an
        // additional path with WFN as the intermediate between two books.
...
CreateOffer::preCompute()
{
    ...
    if (pays_wfn && !gets_wfn)
        cross_type_ = CrossType::IouToWfn;
    else if (gets_wfn && !pays_wfn)
        cross_type_ = CrossType::WfnToIou;
...
CreateOffer::applyGuts (Sandbox& sb, Sandbox& sbCancel)
{
    ...
    {
        WFNAmount reserve = ctx_.view().fees().accountReserve(
