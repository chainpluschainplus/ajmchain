...
static
WFNAmount accountFundsHelper (ReadView const& view,
    AccountID const& id,
    WFNAmount const& amtDefault,
    Issue const& issue,
    FreezeHandling freezeHandling,
    beast::Journal j)
{
    return toAmount<WFNAmount> (
        accountHolds (view, id, issue.currency, issue.account, freezeHandling, j));
}
...
template class FlowOfferStream<WFNAmount, IOUAmount>;
template class FlowOfferStream<IOUAmount, WFNAmount>;

template class TOfferStreamBase<STAmount, STAmount>;
template class TOfferStreamBase<IOUAmount, IOUAmount>;
template class TOfferStreamBase<WFNAmount, IOUAmount>;
template class TOfferStreamBase<IOUAmount, WFNAmount>;
}
