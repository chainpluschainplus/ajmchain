...
protected:
    ...
    struct Flow
    {
        ...
        {
            if (isWFN (order.in) && isWFN (order.out))
                return false;
...
private:
    ...
    Flow
    flow_wfn_to_iou (Amounts const& offer, Quality quality,
        STAmount const& owner_funds, STAmount const& taker_funds,
        Rate const& rate_out);

    Flow
    flow_iou_to_wfn (Amounts const& offer, Quality quality,
        STAmount const& owner_funds, STAmount const& taker_funds,
        Rate const& rate_in);
...
private:
    ...
    TER
    transferWFN (AccountID const& from, AccountID const& to, STAmount const& amount);
...
private:
    ...
    // The amount of WFN that flowed if we were autobridging
    STAmount wfn_flow_;
