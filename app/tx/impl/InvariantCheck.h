...
/**
 * @brief Invariant: A transaction must not create WFN and should only destroy
 * WFN, up to the transaction fee.
 *
 ...
 * and payment channels.  Remove from the total any previous WFN values and add
 * to the total any new WFN values. The net balance of a payment channel is
...
/**
 * @brief Invariant: An account WFN balance must be in WFN and take a value
                     between 0 and SYSTEM_CURRENCY_START drops, inclusive.
 */
class WFNBalanceChecks
{
    bool bad_ = false;
...
/**
 * @brief Invariant: Trust lines using WFN are not allowed.
 */
class NoWFNTrustLines
{
    bool wfnTrustLine_ = false;
