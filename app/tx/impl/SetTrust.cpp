...
SetTrust::preflight (PreflightContext const& ctx)
{
    ...
    {
        JLOG(j.trace()) <<
            "Malformed transaction: specifies WFN as IOU";
...
SetTrust::doApply ()
{
    ...
    // user enough WFN to cover not only the account reserve
    // but the incremental reserve for the trust line as
    // well. A person with no intention of using the gateway
    // could use the extra WFN for their own purposes.

    WFNAmount const reserveCreate ((uOwnerCount < 2)
        ? WFNAmount (zero)
    ...
    if (sleRippleState)
    {
        ...
        else if (bReserveIncrease && mPriorBalance < reserveCreate)
        {
            ...
            // Another transaction could provide WFN to the account and then
