...
/*
    PaymentChannel
        ...
        Payment channels permit off-ledger checkpoints of WFN payments flowing
        in a single direction. A channel sequesters the owner's WFN in its own
    ...
    PaymentChannelCreate
        ...
        Amount
            The amount of WFN to deposit in the channel immediately.
        SettleDelay
    ...
    PaymentChannelClaim
        ...
        Channel
            The 256-bit ID of the channel.
        Balance (optional)
            The total amount of WFN delivered after this claim is processed (optional, not
            needed if just closing).
        Amount (optional)
            The amount of WFN the signature is for (not needed if equal to Balance or just
            closing the line).
        Signature (optional)
...
PayChanCreate::preflight (PreflightContext const& ctx)
{
    ...
    if (!isWFN (ctx.tx[sfAmount]) || (ctx.tx[sfAmount] <= beast::zero))
...
PayChanClaim::preflight (PreflightContext const& ctx)
{
    ...
    if (bal && (!isWFN (*bal) || *bal <= beast::zero))
        return temBAD_AMOUNT;

    auto const amt = ctx.tx[~sfAmount];
    if (amt && (!isWFN (*amt) || *amt <= beast::zero))
        return temBAD_AMOUNT;
    ...
    if (auto const sig = ctx.tx[~sfSignature])
    {
        ...
        auto const reqBalance = bal->wfn ();
        auto const authAmt = amt ? amt->wfn() : reqBalance;
...
PayChanClaim::doApply()
{
    ...
    {
        auto const chanBalance = slep->getFieldAmount (sfBalance).wfn ();
        auto const chanFunds = slep->getFieldAmount (sfAmount).wfn ();
        auto const reqBalance = ctx_.tx[sfBalance].wfn ();
        ...
        if (txAccount == src && ((*sled)[sfFlags] & lsfDisallowWFN))
            return tecNO_TARGET;

        (*slep)[sfBalance] = ctx_.tx[sfBalance];
        WFNAmount const reqDelta = reqBalance - chanBalance;
