...
bool checkNear (WFNAmount const& expected, WFNAmount const& actual)
{
    return expected == actual;
};

static
bool isWFNAccount (STPathElement const& pe)
{
    if (pe.getNodeType () != STPathElement::typeAccount)
        return false;
    return isWFN (pe.getAccountID ());
};
    ...
    Issue const& curIssue)
{
    ...
    if (ctx.isFirst && e1->isAccount () &&
        ...
        isWFN (e1->getCurrency ()))
    {
        return make_WFNEndpointStep (ctx, e1->getAccountID ());
    }

    if (ctx.isLast && isWFNAccount (*e1) && e2->isAccount())
        return make_WFNEndpointStep (ctx, e2->getAccountID());
    ...
    if (isWFN (curIssue.currency) && isWFN (outCurrency))
    {
        JLOG (j.warn()) << "Found wfn/wfn offer payment step";
        return {temBAD_PATH, std::unique_ptr<Step>{}};
    }

    assert (e2->isOffer ());

    if (isWFN (outCurrency))
        return make_BookStepIX (ctx, curIssue);

    if (isWFN (curIssue.currency))
    ...
    beast::Journal j)
{
    if (isWFN (src))
    {
        JLOG (j.debug()) << "toStrand with wfnAccount as src";
        return {temBAD_PATH, Strand{}};
    }
    if (isWFN (dst))
    {
        JLOG (j.debug()) << "toStrand with wfnAccount as dst";
        return {temBAD_PATH, Strand{}};
    }
    ...
    beast::Journal j)
{
    ...
    for (auto const& pe : path)
    {
        ...
        if (hasIssuer && isWFN(pe.getIssuerID()))
            return {temBAD_PATH, Strand{}};

        if (hasAccount && isWFN(pe.getAccountID()))
            return {temBAD_PATH, Strand{}};

        if (hasCurrency && hasIssuer &&
            isWFN(pe.getCurrency()) != isWFN(pe.getIssuerID()))
            return {temBAD_PATH, Strand{}};
    ...
    */
        ...
        */
        ...
        if (cur->isAccount() && next->isAccount())
        {
                ...
                curIssue.account != next->getAccountID ())
            {
                ...
                result.push_back (std::move (msr.second));
                impliedPE.emplace(STPathElement::typeAccount,
                    curIssue.account, wfnCurrency(), wfnAccount());
                cur = &*impliedPE;
            }
        }
        else if (cur->isAccount() && next->isOffer())
        ...
        else if (cur->isOffer() && next->isAccount())
        {
                ...
                !isWFN (next->getAccountID ()))
            {
                if (isWFN(curIssue))
                {
                    ...
                    {
                        // Last step. insert wfn endpoint step
                        auto msr = make_WFNEndpointStep (ctx(), next->getAccountID());
                        if (msr.first != tesSUCCESS)
                            return {msr.first, Strand{}};
                        result.push_back(std::move(msr.second));
                    }
    ...
    auto checkStrand = [&]() -> bool {
        auto stepAccts = [](Step const& s) -> std::pair<AccountID, AccountID> {
            ...
            return std::make_pair(wfnAccount(), wfnAccount());
        };
...
bool
isDirectWfnToWfn(Strand const& strand)
{
    return false;
}
...
template<>
bool
isDirectWfnToWfn<WFNAmount, WFNAmount> (Strand const& strand)
{
...
template
bool
isDirectWfnToWfn<WFNAmount, IOUAmount> (Strand const& strand);
template
bool
isDirectWfnToWfn<IOUAmount, WFNAmount> (Strand const& strand);
template
bool
isDirectWfnToWfn<IOUAmount, IOUAmount> (Strand const& strand);
