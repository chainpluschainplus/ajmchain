...
public:
        ...
        bool prevStepRedeems) const
    {
        ...
        auto rate = [&](AccountID const& id) {
            if (isWFN(id) || id == this->strandDst_)
                return parityRate;
    ...
    Callback& callback) const
{
    ...
    while (offers.step ())
    {
        ...
        // An account can always own WFN or their own IOUs.
        if (flowCross &&
            (!isWFN (offer.issueIn().currency)) &&
...
{
    bool const inWFN = isWFN (book.in.currency);
    bool const outWFN = isWFN (book.out.currency);
    if (inWFN && outWFN)
        return equalHelper<WFNAmount, WFNAmount,
            BookPaymentStep<WFNAmount, WFNAmount>> (step, book);
    if (inWFN && !outWFN)
        return equalHelper<WFNAmount, IOUAmount,
            BookPaymentStep<WFNAmount, IOUAmount>> (step, book);
    if (!inWFN && outWFN)
        return equalHelper<IOUAmount, WFNAmount,
            BookPaymentStep<IOUAmount, WFNAmount>> (step, book);
    if (!inWFN && !outWFN)
...
make_BookStepIX (
    ...
    Issue const& in)
{
    return make_BookStepHelper<IOUAmount, WFNAmount> (ctx, in, wfnIssue());
}
...
make_BookStepXI (
    ...
    Issue const& out)
{
    return make_BookStepHelper<WFNAmount, IOUAmount> (ctx, wfnIssue(), out);
}
