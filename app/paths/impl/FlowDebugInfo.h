...
struct FlowDebugInfo
{
    ...
    to_string (bool writePassInfo) const
    {
        ...
        if (writePassInfo)
        {
            ...
            auto writeNestedWfnAmtList = [&ostr, &writeWfnAmtList](
                std::vector<std::vector<EitherAmount>> const& amts) {
                ostr << '[';
                if (!amts.empty ())
                {
                    writeWfnAmtList(amts[0], '|');
                    for (size_t i = 1, e = amts.size (); i < e; ++i)
                    {
                        ostr << ';';
                        writeWfnAmtList(amts[i], '|');
                    }
                }
                ostr << ']';
            };
            ...
            if (passInfo.nativeIn)
                writeWfnAmtList (passInfo.in);
            else
                writeIouAmtList (passInfo.in);
            ostr << ", out_pass: ";
            if (passInfo.nativeOut)
                writeWfnAmtList (passInfo.out);
            else
                ...
                !passInfo.liquiditySrcIn.back ().empty ())
            {
                ostr << ", l_src_in: ";
                if (passInfo.nativeIn)
                    writeNestedWfnAmtList (passInfo.liquiditySrcIn);
                else
                    writeNestedIouAmtList (passInfo.liquiditySrcIn);
                ostr << ", l_src_out: ";
                if (passInfo.nativeOut)
                    writeNestedWfnAmtList (passInfo.liquiditySrcOut);
                else
...
{
    return {sb.balanceChanges (rv), sb.wfnDestroyed ()};
}
...
balanceDiffsToString (boost::optional<BalanceDiffs> const& bd)
{
    ...
    auto const& wfnDestroyed = bd->second;
    std::ostringstream ostr;
    ostr << ", wfnDestroyed: " << to_string (wfnDestroyed);
    ostr << ", balanceDiffs: ";
