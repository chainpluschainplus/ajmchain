...
RippleCalc::Output RippleCalc::rippleCalculate (
    ...
    // Issuer:
    //      WFN: wfnAccount()
    //  non-WFN: uSrcAccountID (for any issuer) or another account with
    //           trust node.
    STAmount const& saMaxAmountReq,             // --> -1 = no limit.

    // Issuer:
    //      WFN: wfnAccount()
    //  non-WFN: uDstAccountID (for any issuer) or another account with
    //           trust node.
...
TER RippleCalc::rippleCalculate (detail::FlowDebugInfo* flowDebugInfo)
{
    ...
    // nodes.
    // XXX Might also make a WFN bridge by default.
