...
// - If next node is an offer and output is non-WFN then we are the issuer and
//   do not need to push funds.
// - If next node is an offer and output is WFN then we need to deliver funds to
//   limbo.
TER PathCursor::forwardLiquidityForAccount () const
{
    ...
    else if (previousNode().isAccount() && !nextNode().isAccount())
    {
        ...
        if (nodeIndex_)
        {
            // Non-WFN, current node is the issuer.
            JLOG (j_.trace())
                << "forwardLiquidityForAccount: account --> "
                << "ACCOUNT --> offer";
        ...
        else
        {
            ...
            if (pathState_.inReq() >= zero)
            {
                ...
                // Limit WFN by available. No limit for non-WFN as issuer.
                if (isWFN (node().issue_))
                    node().saFwdDeliver = std::min (
                        node().saFwdDeliver,
                        accountHolds(view(),
                            node().account_,
                            wfnCurrency(),
                            wfnAccount(),
                            fhIGNORE_FREEZE, viewJ)); // WFN can't be frozen
            ...
            else
            {
                ...
                // Deliver WFN to limbo.
                resultCode = accountSend(view(),
                    node().account_, wfnAccount(), node().saFwdDeliver, viewJ);
            }
        }
    }
    else if (!previousNode().isAccount() && nextNode().isAccount())
