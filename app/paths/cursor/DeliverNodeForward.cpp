    ...
    bool callerHasLiquidity) const
{
    ...
    while (resultCode == tesSUCCESS && saInAct + saInFees < saInReq)
    {
        ...
        else if (resultCode == tesSUCCESS)
        {
            ...
            if (!isWFN(nextNode().account_))
            {
                // ? --> OFFER --> account
                // Input fees: vary based upon the consumed offer's owner.
                // Output fees: none as WFN or the destination account is the
                // issuer.
                ...
                // Output: Debit offer owner, send WFN or non-XPR to next
                // account.
            ...
            else
            {
                ...
                // fees).
                auto const& id = isWFN(node().issue_) ?
                        wfnAccount() : node().issue_.account;
            ...
            // is redundant.
            if (isWFN (previousNode().issue_.currency)
                || uInAccountID != node().offerOwnerAccount_)
            {
                auto id = !isWFN(previousNode().issue_.currency) ?
                        uInAccountID : wfnAccount();
                resultCode = accountSend(view(),
                    id,
                    node().offerOwnerAccount_,
                    saInPassAct,
