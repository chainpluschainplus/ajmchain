    ...
    path::detail::FlowDebugInfo* flowDebugInfo)
{
    Issue const srcIssue = [&] {
        ...
        if (!isWFN (deliver.issue ().currency))
    ...
    const bool srcIsWFN = isWFN (srcIssue.currency);
    const bool dstIsWFN = isWFN (dstIssue.currency);

    auto const asDeliver = toAmountSpec (deliver);

    // The src account may send either wfn or iou. The dst account may receive
    // either wfn or iou. Since WFN and IOU amounts are represented by different
    // types, use templates to tell `flow` about the amount types.
    if (srcIsWFN && dstIsWFN)
    {
        return finishFlow (sb, srcIssue, dstIssue,
            flow<WFNAmount, WFNAmount> (
                sb, strands, asDeliver.wfn, partialPayment, offerCrossing,
    ...
    {
        return finishFlow (sb, srcIssue, dstIssue,
            flow<IOUAmount, WFNAmount> (
                sb, strands, asDeliver.wfn, partialPayment, offerCrossing,
                limitQuality, sendMax, j, flowDebugInfo));
    }

    assert (!srcIsWFN && !dstIsWFN);
    return finishFlow (sb, srcIssue, dstIssue,
