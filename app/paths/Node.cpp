...
Json::Value Node::getJson () const
{
    ...
    bool const hasCurrency = !isWFN (issue_.currency);
    bool const hasAccount = !isWFN (account_);
    bool const hasIssuer = !isWFN (issue_.account);
    ...
    if (!isWFN (issue_.currency))
        jvNode[jss::currency] = to_string (issue_.currency);

    if (!isWFN (issue_.account))
