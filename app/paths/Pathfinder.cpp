...
The pathfinding request is identified by category, WFN to WFN, WFN to
non-WFN, non-WFN to WFN, same currency non-WFN to non-WFN, cross-currency
non-WFN to non-WFN.  For each category, there is a table of paths that the
...
Pathfinder::Pathfinder (
    ...
    :   mSrcAccount (uSrcAccount),
        ...
        mEffectiveDst (isWFN(saDstAmount.getIssuer ()) ?
        ...
        mSrcAmount (srcAmount.value_or(STAmount({uSrcCurrency,
            uSrcIssuer.value_or(isWFN(uSrcCurrency) ?
                wfnAccount() : uSrcAccount)}, 1u, 0, true))),
...
{
    assert (! uSrcIssuer || isWFN(uSrcCurrency) == isWFN(uSrcIssuer.get()));
}
...
bool Pathfinder::findPaths (int searchLevel)
{
    ...
    auto currencyIsWFN = isWFN (mSrcCurrency);

    bool useIssuerAccount
            = mSrcIssuer && !currencyIsWFN && !isWFN (*mSrcIssuer);
    auto& account = useIssuerAccount ? *mSrcIssuer : mSrcAccount;
    auto issuer = currencyIsWFN ? AccountID() : account;
    ...
    bool bSrcWfn = isWFN (mSrcCurrency);
    bool bDstWfn = isWFN (mDstAmount.getCurrency());
    ...
    if (! mLedger->exists (keylet::account (mDstAccount)))
    {
        ...
        {
            JLOG (j_.debug())
                    << "New account not being funded in WFN ";
            return false;
        }
    ...
    {
        // WFN -> WFN
        JLOG (j_.debug()) << "WFN to WFN payment";
        paymentType = pt_WFN_to_WFN;
    }
    else if (bSrcWfn)
    {
        // WFN -> non-WFN
        JLOG (j_.debug()) << "WFN to non-WFN payment";
        paymentType = pt_WFN_to_nonWFN;
    }
    else if (bDstWfn)
    {
        // non-WFN -> WFN
        JLOG (j_.debug()) << "non-WFN to WFN payment";
        paymentType = pt_nonWFN_to_WFN;
    }
    else if (mSrcCurrency == mDstAmount.getCurrency ())
    {
        // non-WFN -> non-WFN - Same currency
        JLOG (j_.debug()) << "non-WFN to non-WFN - same currency";
        paymentType = pt_nonWFN_to_same;
    }
    else
    {
        // non-WFN to non-WFN - Different currency
        JLOG (j_.debug()) << "non-WFN to non-WFN - cross currency";
        paymentType = pt_nonWFN_to_nonWFN;
    }
    ...
    AccountID const& srcIssuer)
{
    ...
    const bool issuerIsSender = isWFN (mSrcCurrency) || (srcIssuer == mSrcAccount);
...
STPathSet& Pathfinder::addPathsForType (PathType const& pathType)
{
    ...
    case nt_WFN_BOOK:
        addLinks (parentPaths, pathsOut, afADD_BOOKS | afOB_WFN);
        break;
    ...
    int addFlags)
{
    ...
    bool const bOnWFN = uEndCurrency.isZero ();
    ...
    if (addFlags & afADD_ACCOUNTS)
    {
        ...
        if (bOnWFN)
        {
            ...
            { // non-default path to WFN destination
                JLOG (j_.trace())
    ...
    {
        // add order books
        if (addFlags & afOB_WFN)
        {
            // to WFN only
            if (!bOnWFN && app_.getOrderBookDB ().isBookToWFN (
            ...
            for (auto const& book : books)
            {
                     ...
                     (book->getCurrencyOut () == mDstAmount.getCurrency ())))
                {
                    ...
                    { // to WFN
                        ...
                        {
                            // destination is WFN, add account and path is
                            // complete
                            JLOG (j_.trace())
                        ...
                        book->getIssuerOut ()))
                    {
                            ...
                            (newPath[newPath.size() - 2].isOffer ()))
                        {
                            ...
                            newPath[newPath.size() - 1] = STPathElement (
                                ...
                                wfnAccount(), book->getCurrencyOut(),
                                book->getIssuerOut());
                        }
                        else
                        {
...
Pathfinder::PathType makePath (char const *string)
{
    ...
    while (true)
    {
        switch (*string++)
        {
            ...
            case 'x': // wfn book
                ret.push_back (Pathfinder::nt_WFN_BOOK);
                break;
...
void Pathfinder::initPathTable ()
{
    ...
    fillPaths(
        pt_nonWFN_to_WFN, {
            {1, "sxd"},       // gateway buys WFN
            {2, "saxd"},      // source -> gateway -> book(WFN) -> dest
            {6, "saaxd"},
            {7, "sbxd"},
            {8, "sabxd"},
            {9, "sabaxd"}
    ...
    // non-WFN to non-WFN (same currency)
    fillPaths(
        pt_nonWFN_to_same,  {
            ...
            {6, "sxfad"},
            {6, "safad"},
            {6, "saxfd"},   // source -> gateway -> book to WFN -> book ->
                            // destination
            {6, "saxfad"},
