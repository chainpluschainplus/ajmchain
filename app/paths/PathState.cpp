...
//   an exact match.
// - Real issuers must be specified for non-WFN.
    ...
    AccountID const& issuer)    // --> Delivering this issuer.
{
    ...
    if (nodes_.back ().issue_.currency != currency)
    {
        ...
        // book.  wfnAccount() does double duty as signaling "this is an order
        // book".

        // Corresponds to "Implies an offer directory" in the diagram, currently
        // at http://goo.gl/Uj3HAB.

        auto type = isWFN(currency) ? STPathElement::typeCurrency
            : STPathElement::typeCurrency | STPathElement::typeIssuer;

        // The offer's output is what is now wanted.
        // wfnAccount() is a placeholder for offers.
        resultCode = pushNode (type, wfnAccount(), currency, issuer);
    }


    // For ripple, non-WFN, ensure the issuer is on at least one side of the
    // transaction.
    ...
    AccountID const& issuer)     // If not specified, default to previous.
{
    ...
    else if (hasIssuer && isWFN (node.issue_))
    {
        JLOG (j_.debug()) << "pushNode: issuer specified for WFN.";
    ...
    else if (hasAccount)
    {
        ...
        node.issue_.account = hasIssuer ? issuer :
                (isWFN (node.issue_) ? wfnAccount() : account);
        ...
        else
        {
            ...
            resultCode = pushImpliedNodes (
                ...
                isWFN(node.issue_.currency) ? wfnAccount() : account);
    ...
    else
    {
        ...
        else if (isWFN (node.issue_.currency))
            node.issue_.account = wfnAccount();
        else if (isWFN (backNode.issue_.account))
        ...
        else if (backNode.issue_ == node.issue_)
        {
            ...
            resultCode   = pushImpliedNodes (
                wfnAccount(), // Rippling, but offers don't have an account.
    ...
    AccountID const& uSenderID)
{
    ...
    AccountID const& uSenderIssuerID
        = isWFN(uMaxCurrencyID) ? wfnAccount() : uSenderID;
    // Sender is always issuer for non-WFN.
    ...
    // WFN with issuer is malformed.
    if ((isWFN (uMaxCurrencyID) && !isWFN (uMaxIssuerID))
        || (isWFN (currencyOutID) && !isWFN (issuerOutID)))
    {
        JLOG (j_.debug())
            << "expandPath> issuer with WFN";
        terStatus   = temBAD_PATH;
    }

    // Push sending node.
    // For non-WFN, issuer is always sending account.
    // - Trying to expand, not-compact.
    // - Every issuer will be traversed through.
    if (terStatus == tesSUCCESS)
    {
        terStatus   = pushNode (
            !isWFN(uMaxCurrencyID)
    ...
    if (tesSUCCESS == terStatus && uMaxIssuerID != uSenderIssuerID)
    {
        ...
        // - If it was WFN, then issuers would have matched.
        ...
        // account.
        if (!uNxtCurrencyID
            // Next is WFN, offer next. Must go through issuer.
    ...
    if (terStatus == tesSUCCESS
        && !isWFN(currencyOutID)               // Next is not WFN
