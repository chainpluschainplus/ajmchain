...
public:
    ...
    */
	...
	void pubAjmChainTableTxs(const AccountID& ownerId, const std::string& sTableName, 
...
void NetworkOPsImp::apply (std::unique_lock<std::mutex>& batchLock)
{
...

    {
        ...
        for (TransactionStatus& e : transactions)
        {
            ...
            {
                if (e.transaction->getSTransaction()->isAjmChainTableType())
                {
                    addLocal = false;
                }
                JLOG(m_journal.debug())
			...
			//ajmchain type tx will not retry.
            if (addLocal && !e.transaction->getSTransaction()->isAjmChainTableType())
            {
                m_localTX->push_back (
...
void NetworkOPsImp::PubValidatedTxForTable(const STTx& tx)
{
	...
	if (vecTxs.size() > 1)
	{
		...
		{
			pubAjmChainTableTxs(item.first, item.second, tx, res);
		}
		if (listPair.size() > 0)
		{
	...
	const STTx& stTxn, const std::pair<std::string, std::string>& res,bool bValidated)
{
	...
	pubAjmChainTableTxs(owner, sTableName, stTxn, res);
}
	...
	if (!subTx.empty())
	{
		...
		if (simiIt != subTx.end())
		{
			...
			if (p)
			{
				...
				if (bValidated) {
					if (bForTableTx) {
						//for ajmchain type, subscribe db event
...
void NetworkOPsImp::pubAjmChainTableTxs(const AccountID& ownerId, const std::string& sTableName, 
	const STTx& stTxn,const std::pair<std::string,std::string>& disposRes)
{
	ScopedLockType sl(mSubLock);
