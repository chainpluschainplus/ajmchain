...
{
    bool toWFN = isWFN (book.out);
    std::lock_guard <std::recursive_mutex> sl (mLock);

    if (toWFN)
    {
        // We don't want to search through all the to-WFN or from-WFN order
        // books!
        for (auto ob: mSourceMap[book.in])
        {
            if (isWFN (ob->getCurrencyOut ())) // also to WFN
                return;
        }
    }
    else
...
bool OrderBookDB::isBookToWFN(Issue const& issue)
{
    std::lock_guard <std::recursive_mutex> sl (mLock);
    return mWFNBooks.count(issue) > 0;
}
