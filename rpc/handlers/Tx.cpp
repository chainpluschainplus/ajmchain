...
namespace ripple {
    ...
    {
        if (!STTx::checkAjmchainTableType(txType) && !STTx::checkAjmchainContractType(txType))  return false;
        ...
        boost::optional<std::string> previousTxid, nextTxid, ownerRead, nameRead, typeRead;
        {
            ...
            while (st.fetch())
            {
                ...
                *db << sSqlPrevious, soci::into(previousTxid);
                ...
                if (isAjmchainContractType(*typeRead))
                {
...
Json::Value doTxCount(RPC::Context& context)
{
	...
	ret["ajmchain"] = context.app.getMasterTransaction().getTxCount(true);
