...
Json::Value doBookOffers (RPC::Context& context)
{
    ...
    {
        pay_issuer = wfnAccount ();
    }

    if (isWFN (pay_currency) && ! isWFN (pay_issuer))
        return RPC::make_error (
            rpcSRC_ISR_MALFORMED, "Unneeded field 'taker_pays.issuer' for "
            "WFN currency specification.");
    ...
    {
        get_issuer = wfnAccount ();
    }


    if (isWFN (get_currency) && ! isWFN (get_issuer))
        return RPC::make_error (rpcDST_ISR_MALFORMED,
    ...
    if (!isWFN (get_currency) && isWFN (get_issuer))
        return RPC::make_error (rpcDST_ISR_MALFORMED,
            "Invalid field 'taker_gets.issuer', expected non-WFN issuer.");
